...
The backend container is multi-stage, deterministic (`npm ci`), non-root, and exposes `/health` and database-aware `/ready`. `COOKIE_SECURE=true`, `MALWARE_SCAN_REQUIRED=true`, explicit `CORS_ORIGIN`, short access-token lifetime, and high-entropy `JWT_SECRET` are mandatory in production. Do not expose model, embedding, storage, scanner, extractor, database, or SyteLine endpoints directly to browsers.
