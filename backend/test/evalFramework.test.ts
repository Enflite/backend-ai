...
describe('eval routes', () => {
  ...
  it('POST /admin/eval/runs runs the mock suite and persists results', async () => {
    ...
    // Default corpus is the full 140-case suite; the 2 llm-judge cases skip
    // without a judge model (reported in `skipped`, excluded from totals,
    // never gated). saveCaseResult still persists one row per case.
    expect(body.summary.total).toBe(138);
    expect(body.summary.passed).toBe(138);
    expect(body.summary.skipped).toBe(2);
    // One row per case, including skipped ones.
    expect(vi.mocked(saveCaseResult).mock.calls).toHaveLength(140);
    expect(vi.mocked(finishRun)).toHaveBeenCalledWith('run-1', expect.objectContaining({ total: 138 }));
    await app.close();
  });
